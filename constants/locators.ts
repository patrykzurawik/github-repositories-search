export const LocatorAppTitle = 'app-title';
export const LocatorAppLogo = 'app-logo';

export const LocatorFormFieldWrapper = 'form-field-wrapper';
export const LocatorFormFieldText = 'form-field-text';

export const LocatorButton = 'button';
export const LocatorButtonPrimary = 'button-primary';
export const LocatorButtonSecondary = 'button-secondary';

export const LocatorAlert = 'alert';
export const LocatorAlertWarning = 'alert-warning';
export const LocatorAlertError = 'alert-error';

export const LocatorForm = 'form';

export const LocatorList = 'list';

export const LocatorSearchForm = 'search-form';

export const LocatorSearchList = 'search-list';
